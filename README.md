...
In diesem Stack ist Prometheus der Speicher für deiner Anwendungsmetriken. Der Datenfluss sieht so aus:

1. Die FastAPI-App erzeugt Metriken (z.B. einen Zähler für verarbeitete Tasks) mit dem OpenTelemetry SDK.

2. Alloy empfängt diese Metriken über das OTLP-Protokoll.

3. Alloy leitet die Metriken per remote_write an Prometheus weiter.

4. Grafana fragt Prometheus ab, um die Metriken zu visualisieren.

## Welche Code-Änderungen waren nötig?

Um die Metriken von der App zu Prometheus zu bekommen, waren zwei Änderungen entscheidend, die im
Branch `feat/api-loki-alloy-grafana-tempo-otel-metrics` implementiert sind:

- In `app.py` (Python-Anwendung): Dem manuell erstellten MeterProvider wurde ein PeriodicExportingMetricReader mit einem
  `OTLPMetricExporter` hinzugefügt. Die OTEL_* Umgebungsvariablen konfigurieren nur die automatische Instrumentierung.
  Für
  manuell erstellte Metriken, wie den `tasks_processed_counter`, musst du dem Code explizit sagen, wie und wohin er
  die Metriken exportieren soll. Ohne diesen Codeblock erzeugt die Anwendung zwar Metriken, sendet sie aber nie ab.

- In `docker-compose.yaml`kommt der Service Prometheus dazu. Außerdem wird das Startkommando des Prometheus-Containers
  um das Flag --web.enable-remote-write-receiver erweitert. Standardmäßig ist Prometheus darauf ausgelegt, Metriken von
  Zielen aktiv abzufragen (Scraping). In unserem Aufbau sendet Alloy die Metriken jedoch aktiv an Prometheus (Pushing).
  Dieses Flag aktiviert den Endpunkt, an dem Prometheus diese gesendeten Daten empfangen kann. Ohne es würde Prometheus
  die Daten von Alloy einfach ablehnen.

## 🛠️ Live Debugging mit Alloy - otel, loki, tempo & prometheus

Im alloy Graph kannst du nun live über die Komponente `prometheus.remote_write.to_prometheus` rohe Metrik-Daten
einsehen.
...
```
zeigt beispielhaft, welche Informationen du aus den rohen Daten ableiten kannst:
